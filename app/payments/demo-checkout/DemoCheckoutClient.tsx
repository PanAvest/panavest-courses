...
export default function DemoCheckoutClient() {
  ...
  // Require auth; if not, redirect to sign-in then bounce back here
  useEffect(() => {
    let active = true;
    (async () => {
  ...
  return (
    <main className="mx-auto max-w-screen-sm px-4 py-10">
      ...
      <p className="mt-2 text-muted">Item: <span className="font-medium">{slug}</span></p>
      ...
      <p className="mt-4 text-xs text-muted">
        Demo flow writes <code>paid</code> to <code>ebook_purchases</code> for your account.
      </p>
    </main>
  );
}
